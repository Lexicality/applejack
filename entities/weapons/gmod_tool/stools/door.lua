...
if (SERVER) then
	if (not ConVarExists("sbox_maxdoors")) then
		CreateConVar(
			"sbox_maxdoors", GM.Config["Maximum Doors"], {FCVAR_NOTIFY, FCVAR_REPLICATED}
		);
	else
	...
	language.Add(
		"Tool_door_0",
		"Click somewhere to spawn a door. This will cost $" .. GM.Config["Door Cost"]
	)
...
local function refund(ent)
	...
	owner:Notify(
		"You got $" .. GM.Config["Door Cost"] / 2 .. " for selling your door.", 0
	);
	owner:TakeDoor(ent);
end
...
function TOOL.BuildCPanel(CPanel)
	...
	-- HEADER
	CPanel:AddControl(
		"Header", {Text = "#Tool_door_name", Description = "#Tool_door_desc"}
	);
	...
	params.Options["Tall Combine Door"] = {
		door_class = "prop_dynamic",
		door_model = "models/props_combine/combine_door01.mdl",
	}
	params.Options["Lab Elevator Door"] = {
		door_class = "prop_dynamic",
		door_model = "models/props_lab/elevatordoor.mdl",
	}
	params.Options["Wide Combine Door"] = {
		door_class = "prop_dynamic",
		door_model = "models/combine_gate_Vehicle.mdl",
	}
	params.Options["Thin Combine Door"] = {
		door_class = "prop_dynamic",
		door_model = "models/combine_gate_citizen.mdl",
	}
	params.Options["Secret Lab Door"] = {
		door_class = "prop_dynamic",
		door_model = "models/props_doors/doorKLab01.mdl",
	}
	params.Options["Door with a handle"] = {
		door_class = "prop_door_rotating",
		door_model = "models/props_c17/door01_left.mdl",
		door_hardware = "1",
	}
	params.Options["Door with a bar"] = {
		door_class = "prop_door_rotating",
		door_model = "models/props_c17/door01_left.mdl",
		door_hardware = "2",
	}
...
function TOOL:Think()
	...
	if (not self.GhostEntity or not self.GhostEntity:IsValid() or
		self.GhostEntity:GetModel() ~= self:GetClientInfo("model")) then
		self:MakeGhostEntity(
			self:GetClientInfo("model"), Vector(0, 0, 0), Angle(0, 0, 0)
		)
	end
