...
function SWEP:Initialize()
	...
	self.FireFunction = self.FireModes[self.AvailableFireModes[self.CurFireMode]]
                    		.FireFunction
	self.dt.firemode = 1;
...
function SWEP:Deploy()
	if (SERVER) then
		if (not self.DrawCrosshair or self.CustomCrosshair) then
...
function SWEP:StickAGunToMyBack()
	...
	local pos, ang, up, forward = player:GetPos(), player:GetAngles(),
                              	player:GetUp(), player:GetForward()
...
function SWEP:DoAmmoStuff()
	...
	if not (self.previousOwner._Ammo[self.ClassName] == 0 and
		self.previousOwner:GetAmmoCount(self.Primary.Ammo) == 0) -- Make sure the player is alive
	or not self.previousOwner:Alive() -- Make sure the player can actually put this weapon in their inventory
	or not GAMEMODE.Items[self.ClassName] -- Make sure we haven't done this already
	or self.doneAmmoStuff -- Make sure the owner isn't already holstering
	or self.previousOwner._Equipping then
		return
	end
...
function SWEP:Holster(wep)
	...
	-- 	return false
	-- end
	if SERVER and IsValid(self.Owner) and self.Owner:Alive() and
...
function SWEP:OnDrop()
	...
	if IsValid(self.previousOwner) and self.previousOwner._GunCounts then
		self.previousOwner._GunCounts[self.Size] =
			self.previousOwner._GunCounts[self.Size] or 1
		self.previousOwner._GunCounts[self.Size] =
			self.previousOwner._GunCounts[self.Size] - 1
	end
	self:DoAmmoStuff()
	self:ResetVars()
...
function SWEP:BaseAttack()
	...
	if not self:CanFire(self.Weapon:Clip1()) then
		return
	end
	...
	self.CurrentSpread = math.Clamp(
		self.CurrentSpread * RecoverScale, self.MinSpread, self.MaxSpread
	)
	self.CurrentRecoil = math.Clamp(
		self.CurrentRecoil * RecoverScale, self.MinRecoil, self.MaxRecoil
	)
	self.CurrentSpray = math.Clamp(
		self.CurrentSpray * RecoverScale, self.MinSpray, self.MaxSpray
	)
	self.SprayVec = self.SprayVec *
                		((self.CurrentSpray - self.MinSpray) /
                			(self.MaxSpray - self.MinSpray))

	-- Calculate modifiers/take ammo
	local modifier = 1
	if not self.OwnerIsNPC then -- NPCs don't get modifiers

		modifier = self:CalculateModifiers(
			self.RunModifier, self.CrouchModifier, self.JumpModifier,
			self.IronSightModifier
		)
	...
	self:RGShootBulletCheap(
		self.Primary.Damage, self.BulletSpeed, self.CurrentSpread * modifier,
		NewSpray, self.SprayVec
	)
	...
	self.CurrentRecoil = math.Clamp(
		self.CurrentRecoil + self.DeltaRecoil, self.MinRecoil, self.MaxRecoil
	)
	self.CurrentSpread = math.Clamp(
		self.CurrentSpread + self.DeltaSpread, self.MinSpread, self.MaxSpread
	)
	self.CurrentSpray = math.Clamp(
		self.CurrentSpray + self.DeltaSpray, self.MinSpray, self.MaxSpray
	)
...
local function carshoot(self, tr, dmg)
	if tr.Entity:IsValid() and tr.Entity:GetClass() == "prop_vehicle_jeep" and
		SERVER then
		local trace = {
			start = tr.HitPos,
			endpos = tr.HitPos + tr.Normal * 64,
			filter = tr.Entity,
		}
		trace = util.TraceLine(trace)
		if trace.Hit and IsValid(trace.Entity) then
			local ent = trace.Entity
			if ent:IsVehicle() and IsValid(ent:GetDriver()) and ent:GetClass() ==
				"prop_vehicle_prisoner_pod" then
...
function SWEP:RGShootBulletCheap(dmg, speed, spread, spray, sprayvec, numbul)
	...
	for _, trace in ipairs(traces) do
		self:PenetrateCallback(
			self.Owner, trace, self.PenetrateInfo.Damage, self.PenetrateInfo.Force
		)
	end
end
local ricochetmats = {}
...
function SWEP:PenetrateCallback(pl, trace, damage, force)
	...
	if (not mats) then
		ErrorNoHalt(
			"Unkown material type '" .. trace.MatType .. "' for ricochet callback! o_O"
		);
		mats = -0.1
	end
	if -(trace.Normal:Dot(trace.HitNormal)) < mats and
		hook.Call("CanRicochet", GAMEMODE, trace, force, self) then
		if game.SinglePlayer() then
			debugoverlay.Text(trace.HitPos + vector_up, "ricochet", 10)
		end
		dir = bounce(trace) + VectorRand() / 4
		damage, force = damage * 0.75, force / 2
	elseif self.CanPenetrate and (not trace.HitWorld or self.CanPenetrateWorld) and
		hook.Call("CanPenetrate", GAMEMODE, trace, force, self) and
		not self.dt.silenced then
		if game.SinglePlayer() then
	...
	bullet.Callback = function(atk, _trace, di)
		...
		if game.SinglePlayer() then
			debugoverlay.Line(
				_trace.StartPos, _trace.HitPos, 10, Color(255, 237, 31, 255), true
			)
			debugoverlay.Line(
				trace.HitPos, _trace.HitPos, 10, Color(200, 200, 255, 255), true
			)
		end
		tr = _trace
		...
		if (not IsValid(_trace.Entity) or
			(not _trace.Entity:IsPlayer() and not _trace.Entity:IsNPC())) then
			if (_trace.Entity == trace.Entity) then
				if (matTypes[_trace.MatType]) then
					util.Decal(
						matTypes[_trace.MatType][1], _trace.HitPos + _trace.HitNormal,
						_trace.HitPos - _trace.HitNormal
					)
...
function SWEP:ApplyRecoil(recoil, spray)
	...
	if self.OwnerIsNPC or (SERVER --[[ and not self.Owner:IsListenServerHost()]] ) then
		return
	end
...
function SWEP:ShootEffects()
	...
	if self.dt.silenced then
		...
		if (self.AnimationFix) then
			timer.Create(
				"Animation Fix: " .. tostring(self.Weapon), self.AnimationFix, 1, function()
...
function SWEP:CanFire(clip)
	...
	if not self.Weapon or not self.Owner or
		not (self.OwnerIsNPC or self.Owner:Alive()) then
		return
	end
...
function SWEP:SetIronsights(b, player)
	...
	if CLIENT or (not player) or player:IsNPC() then
		return
	end
...
function SWEP:SecondaryAttack()
	...
	if self.NextSecondaryAttack > CurTime() or self.OwnerIsNPC then
		return
	end
	...
	if self.Owner:KeyDown(IN_USE) then
		...
		if NumberOfFireModes < 2 then
			return
		end -- We need at least 2 firemodes to change firemodes!
