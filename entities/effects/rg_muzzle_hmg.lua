function EFFECT:Init(data)
	...
	self.Position = self:GetTracerShootPos(
		data:GetOrigin(), self.WeaponEnt, self.Attachment
	)
	...
	if not IsValid(self.WeaponEnt) or not self.WeaponEnt:GetOwner() or
		self.WeaponEnt == NULL or self.WeaponEnt:GetOwner() == NULL then
		return
	end
	...
	local particle = emitter:Add(
		"sprites/heatwave", self.Position - self.Forward * 4
	)
	...
	for i = 1, 4 do
		...
		particle:SetVelocity(
			math.Rand(50, 90) * self.Forward + 40 * VectorRand() + 1.1 * AddVel
		)
	...
	if math.random(1, 3) == 1 then
		for j = 1, 2 do
			for i = -1, 1, 2 do
				local particle = emitter:Add(
					"effects/muzzleflash" .. math.random(1, 4),
					self.Position - 3 * self.Forward + 2 * j * i * self.Right
				)
	...
	if math.random(1, 3) == 1 then
		for j = 1, 2 do
			for i = -1, 1, 2 do
				local particle = emitter:Add(
					"effects/muzzleflash" .. math.random(1, 4),
					self.Position - 3 * self.Forward + 2 * j * i * self.Up
				)
