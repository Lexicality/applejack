function EFFECT:Init(data)
	...
	if not (self.WeaponEnt:IsValid() and self.WeaponEnt:IsWeapon()) then
		return
	end
	...
	if self.WeaponEnt:IsCarriedByLocalPlayer() and GetViewEntity() == LocalPlayer() then
		...
		if not ViewModel:IsValid() then
			return
		end

		self.EjectionPort = ViewModel:GetAttachment(self.Attachment)
		if not self.EjectionPort then
			return
		end
		...
		self.Position = self.EjectionPort.Pos -
                			(0.5 * self.WeaponEnt:BoundingRadius()) *
                			self.EjectionPort.Ang:Forward()
	...
	for i = 1, 3 do
		local particle = emitter:Add(
			"particle/particle_smokegrenade", self.Position + self.Forward * 2
		)
