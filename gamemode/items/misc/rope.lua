...
function ITEM:onUse(player)
	...
	if (not (ValidEntity(target) and target:IsPlayer() and player:GetPos():Distance(trace.HitPos) <= 128)) then
	...
	timer.Conditional(player:UniqueID().." tying timer", GM.Config["Tying Timeout"], conditional, success, failure, player, target, player:GetPos(), target:GetPos());
end