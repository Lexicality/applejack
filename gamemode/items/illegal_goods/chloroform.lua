...
function ITEM:onUse(ply)
	...
	victim = tr.Entity;
	if (not (IsValid(victim) and victim:IsPlayer() and victim:Alive() and
		not victim:KnockedOut() and ply:GetShootPos():Distance(tr.HitPos) <= 80 and
		math.abs(victim:GetAimVector():Angle().y - tr.Normal:Angle().y) < 35)) then
