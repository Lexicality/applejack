...
function GM:PlayerCanSpawnProp(ply, mdl)
	...
	elseif ((ply._NextSpawnProp or 0) > time) then
		ply:Notify(
			"You must wait " .. math.ceil(ply._NextSpawnProp - time) ..
				" second(s) before spawning another prop!", 1
		);
		return false;
	end
...
function GM:PlayerSpawnProp(ply, model)
	return (ply:IsAdmin() or ply:HasAccess("e")) and
       		gamemode.Call("PlayerCanSpawnProp", ply, model);
end
...
function GM:PlayerInitialized(ply)
	if (ply.cider._Donator and ply.cider._Donator > 0) then
		...
		if (expire > 0) then
			...
			local minutes =
				string.format("%02.f", math.floor(expire / 60 - (hours * 60)))
			local seconds = string.format(
				"%02.f", math.floor(expire - hours * 3600 - minutes * 60)
			)
			...
			if (days > 0) then
				...
				ply:Notify(
					"Your Donator status expires in " .. hours .. " hour(s) " .. minutes ..
...
function GM:PlayerDataLoaded(ply, success)
	...
	ply:SetNWBool(
		"Moderator", ply:IsUserGroup("operator") or ply:IsUserGroup("moderator") or
			(evolve and ply:EV_GetRank() == "moderator") or
...
function GM:PlayerSetModel(ply)
	...
	-- Query the team in case they're not in one we know of.
...
function GM:PlayerDeath(ply, inflictor, attacker, ragdoll, fall)
	...
	if (attacker:IsPlayer()) then
		pvp, text1, text2, formattext = true, attacker:Name(), ply:Name(),
                                		"%s killed %s"
		if (IsValid(attacker:GetActiveWeapon())) then
			formattext, text3 = formattext .. " with a %s.",
                    			attacker:GetActiveWeapon():GetClass()
		else
			formattext = formattext .. "."
		end
	elseif (attacker:IsVehicle()) then
		local formattext, text1, text2 = "%s was run over by a %s", ply:Name(),
                                 		attacker:GetClass();
		...
		if (IsValid(attacker:GetDriver()) and attacker:GetDriver():IsPlayer()) then
			pvp = true
			formattext, text3 = formattext .. " driven by %s",
                    			attacker:GetDriver():Name()
		end
	elseif fall then
	...
	elseif attacker:GetClass() == "prop_physics" then
		formattext, text2 = "%s was killed with a physics object. (%s)",
                    		attacker:GetModel()
	else
...
function GM:EntityTakeDamage(entity, damageInfo)
	...
	-- 	return
	-- end
	local inflictor, attacker, amount = damageInfo:GetInflictor(),
	...
	if (attacker:IsPlayer() and
		(attacker:GetMoveType() == MOVETYPE_NOCLIP or attacker._StuckInWorld)) or
		(entity:IsPlayer() and entity:GetMoveType() == MOVETYPE_NOCLIP and
			not entity:InVehicle()) or (entity:IsPlayer() and entity._Physgunnin) then
		damageInfo:SetDamage(0)
		return false
	end
	local asplode = false
	...
	if (entity:IsPlayer()) then
		if (entity:KnockedOut()) then
			...
			-- :/ hacky
			if attacker:IsVehicle() and attacker:GetClass() ~=
			...
			if entity:InVehicle() then
				if damageInfo:IsExplosionDamage() and
					(not damageInfo:GetDamage() or damageInfo:GetDamage() == 0) then
			...
			if attacker:GetClass():find "cider" or self.Config["Anti propkill"] and
				not damageInfo:IsFallDamage() and attacker:GetClass():find("prop_physics") then
				damageInfo:SetDamage(0)
				return
			end

			-- Check if the player has a last hit group defined.
			if entity._LastHitGroup and (not attacker:IsPlayer() or
				(IsValid(attacker:GetActiveWeapon()) and
					attacker:GetActiveWeapon():GetClass() ~= "cider_hands")) then
				if (entity._LastHitGroup == HITGROUP_HEAD) then
					damageInfo:ScaleDamage(self.Config["Scale Head Damage"])
				elseif (entity._LastHitGroup == HITGROUP_CHEST or entity._LastHitGroup ==
					HITGROUP_GENERIC) then
					damageInfo:ScaleDamage(self.Config["Scale Chest Damage"])
				elseif (entity._LastHitGroup == HITGROUP_LEFTARM or entity._LastHitGroup ==
					HITGROUP_RIGHTARM or entity._LastHitGroup == HITGROUP_LEFTLEG or
					entity._LastHitGroup == HITGROUP_RIGHTLEG or entity._LastHitGroup ==
					HITGROUP_GEAR) then
			...
			if entity:InVehicle() then
				...
				if (entity:Health() <= 0 and entity:Alive()) then
					...
					hook.Call(
						"PlayerDeath", GAMEMODE, entity, inflictor, attacker,
						damageInfo:IsFallDamage()
					)
				end
			end
	...
	elseif (entity:IsNPC()) then
		if (attacker:IsPlayer() and IsValid(attacker:GetActiveWeapon()) and
			attacker:GetActiveWeapon():GetClass() == "weapon_crowbar") then
	...
	elseif (IsValid(entity._Player) and not entity._Corpse) then
		...
		if (not attacker:IsPlayer()) then
			...
			elseif (attacker == game.GetWorld()) then
				if ((entity._NextWorldDamage and entity._NextWorldDamage > CurTime()) or
					damageInfo:GetDamage() <= 10) then
					return
				end
		...
		for i = 1, 2 do
			...
			trace.endpos = trace.start +
               				(damageInfo:GetDamageForce() + (VectorRand() * 16) * 128)
			trace.filter = entity
			...
			util.Decal(
				"Blood", trace.HitPos + trace.HitNormal, trace.HitPos - trace.HitNormal
			)
		end
		...
		if (ply:Health() <= 0 and ply:Alive()) then
			...
			hook.Call(
				"PlayerDeath", GAMEMODE, ply, inflictor, attacker, damageInfo:IsFallDamage()
			)
		end
		entity = ply
		logme = true
...
function GM:PlayerLoadout(ply)
	if (ply:HasAccess("t")) then
		ply:Give("gmod_tool")
	end
	if (ply:HasAccess("p")) then
...
function GM:ShowTeam(ply)
	...
	if not (IsValid(ent) and ent:IsOwnable() and
...
timer.Create(
	"Earning", GM.Config["Earning Interval"], 0, function()
		...
		for k, v in pairs(cplayers) do
			if (IsValid(k) and k:IsPlayer() and
				hook.Call("PlayerCanEarnContraband", GAMEMODE, k)) then
				if (v.refill > 0) then
