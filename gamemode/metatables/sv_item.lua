...
if (not meta) then
	ErrorNoHalt(
			...
			"Applejack: Error setting up serverside item metatable - Item metatable does not exist!"
	);
	return
end
...
function meta:Use(ply)
	if (not self.onUse or
		not (ply.cider._Inventory[self.UniqueID] and
			ply.cider._Inventory[self.UniqueID] > 0) or self:onUse(ply) == false) then
		return false;
	end
...
function meta:Drop(ply, pos, amt)
	if (not self.onDrop or
		not (ply.cider._Inventory[self.UniqueID] and
			ply.cider._Inventory[self.UniqueID] > 0) or self:onDrop(ply, pos, amt) ==
		false) then
		return false;
	end
...
function meta:Sell(ply)
	if (not self.onSell or
		not (ply.cider._Inventory[self.UniqueID] and
			ply.cider._Inventory[self.UniqueID] > 0) or self:onSell(ply) == false) then
		return false;
	end
...
function meta:Pickup(ply)
	if (not self.onPickup or
		not (ply.cider._Inventory[self.UniqueID] and
			ply.cider._Inventory[self.UniqueID] > 0) or self:onPickup(ply) == false) then
		return false;
	end
	return true;
end
