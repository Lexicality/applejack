...
surface.CreateFont(
	font,
	{font = "Tahoma", size = 12, weight = 400, antialias = true, additive = false}
);
-- http://facepunch.com/showthread.php?t=1220579
surface.CreateFont(
	"TabLarge", {font = "Tahoma", size = 13, weight = 700, shadow = true}
);
...
do
	...
	local h1, h2
	GM:AddHUDBar(
		"Health: 100", Color(255, 0, 0), function(self)
			h1 = lpl:Health();
			self.text = "Health: " .. h1;
			h2 = math.min(h1, 100);
	...
	GM:AddHUDBar(
		"Ammoz: nil", Color(0, 0, 255), function(self)
			...
			txt = clip .. "/" .. max .. " (" ..
      				lpl:GetAmmoCount(wep:GetPrimaryAmmoType()) .. ")";
			self.text = txt;
	...
	do -- Job Timer
		...
		-- csvars.Hook(
		GM:AddHUDBar(
			"Job Timer: 00:00", color_orange, function(bar)
...
do
	...
	local boxwidth = surface.GetTextSize(
	...
	local function dynamicbox(box) -- Boxes that can change their contents (countdowns etc)
		...
		surface.SetTextPos(
			sidewidth + (boxwidth - surface.GetTextSize(text)) / 2, by + texty
		);
		paintbox(box, text);
		by = by + boxheight + 2;
	end
	local function staticbox(box) -- Boxes with static text (You are wearing kevlar etc)
		if (box.todraw and not box:todraw()) then
			return
		end -- Allow even static boxes to hide themselves.
	...
	function GM:AddDynamicHUDBox(textfunc, icon)
		dyboxes[#dyboxes + 1] = {text = textfunc, icon = icon and Material(icon)}
	end
	...
	-- Tied
	GM:AddStaticHUDBox(
		"You have been tied up.", "icon16/link.png", function(box)
			return lpl:Tied();
		end
	);
...
do
	...
	hook.Add(
		"LibrariesLoaded", "CSVars shit for teh hud", function()

			do -- Sleepering
				local endtime, length, left;
				local function sleepfunc()
					endtime = lpl._GoToSleepTime;
					if (not (length and endtime and endtime > 0)) then
						return false;
					end
					left = endtime - ctime;
					if (left < 0) then
				...
				csvars.Hook(
					"_GoToSleepTime", "CentreBar", function(ends)
						if (ends == 0) then
							return;
						end
						length = ends - CurTime();
			...
			do -- Stuck! (Bit of a cheat but hey)
				...
				csvars.Hook(
					"_StuckInWorld", "Centrebar", function(stuck)
						active = stuck;
						if (active) then
			...
			do -- Reganing Consiousness
				...
				local function callback()
					...
					if (not (length and endtime)) then
						return false;
					end
					left = endtime - ctime;
					if (left < 0) then
				...
				local barcolor = Color(150, 210, 20);
				csvars.Hook(
					"_WakeUpTime", "CentreBar", function(ends)
						length = ends - CurTime();
			...
			do -- /fallover recovery
				...
				local function timr()
					...
					if (not lpl:KnockedOut()) then
						return timer.Simple(0.01, timr);
					end
...
function GM:HUDPaint()
	...
	if (gamemode.Call("HUDShouldDraw", "MSInfoBoxes")) then
		drawboxes();
	end
	if (gamemode.Call("HUDShouldDraw", "MSHints")) then
		drawhints();
	end
...
function GM:HUDShouldDraw(name)
	...
	elseif (name == "CHudHealth" or name == "CHudBattery" or name ==
		"CHudSuitPower" or name == "CHudAmmo" or name == "CHudSecondaryAmmo") then
		return false;
	end
...
do
	...
	local vehicles;
	hook.Add(
		"Tick", "ESP Vehicle Ticking", function()
			vehicles = {};
			local i = 1;
	...
	-- Vars
	local tr, cent, fdist, dist, class, pos, spos, lpos, cam, alpha, centre, x, y,
		lines, db;
	function GM:HUDPaintESP()
		...
		-- Vars
		tr, cent, fdist, dist, class, pos, spos, lpos, cam, alpha, centre, x, y, lines =
			nil;
		-- Unchanging
						...
						tr = util.TraceLine(
							{
								...
								mask = CONTENTS_SOLID + CONTENTS_MOVEABLE + CONTENTS_OPAQUE +
									CONTENTS_DEBRIS + CONTENTS_HITBOX + CONTENTS_MONSTER,
							}
						);
					end
					-- Visiblity check
