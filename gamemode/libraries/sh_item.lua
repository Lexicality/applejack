...
function GM:LoadItems()
	...
	for plugin, path in pairs(plugins) do
		...
		for _, filename in pairs(folders) do
			if (not (validfile(filename) and not filename:find(".", 1, true)) or filename ==
				"base") then
				continue;
			end
			spath = path .. filename
			...
			if (file.Exists(spath .. "/init.lua", "LUA")) then
				...
				if (not CAT) then
					ErrorNoHalt(
						"Can't find non existant category ", uid, " in plugin ", plugin.Name,
						"!\n"
					);
					continue;
				end
			...
			total = total + count;
			MsgN(
				"   ", verb, " category '", CAT.Name, "' with ", count, " items:\n    ", str
			);
		end
	end
	ITEM, CAT = nil;
...
if SERVER then
	concommand.Add(
		"cider_reload_items", function(ply)
