...
player.updateIgnoreKeys = {_Key = true, _SteamID = true, _UniqueID = true}
...
player.loadFunctions = {
	_Inventory = function(ply, data)
		string.gsub(
			data, "([^;%s]+): ([0-9]+)", function(item, amount)
				if (GM.Items[item]) then
					cider.inventory.update(ply, item, tonumber(amount), true);
				else
					ErrorNoHalt(
						"Ignoring " .. amount .. " nonexistant '" .. item .. "' items in " ..
							ply:Name() .. "'s inventory.\n"
					);
				end
			end
		)
...
player.saveIgnoreKeys = {_Key = true}
...
player.saveEscapeKeys = {_Name = true, _Clan = true}
...
function player.SaveAll(now)
	...
	timer.Create(
		"Saving All Player Data", math.ceil(#plys / 5), 0, saveallTimer, plys
	);
end
