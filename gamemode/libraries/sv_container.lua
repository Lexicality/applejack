...
function cider.container.getContents(entity, player, forMenu)
	if not entity then
		error("no entity passed to cider.container.getContents!", 2)
	end
	if not cider.container.isContainer(entity) then
		return nil
	end
	if player then
		local contents, io, filter = hook.Call(
			"PlayerGetContainerContents", GAMEMODE, entity, player, forMenu
		)
		if contents then
...
function cider.container.getSpaceLeft(entity, player)
	...
	return cider.container.getLimit(entity) -
       		cider.container.getContentsSize(entity, player)
end
...
function cider.container.update(entity, id, amount, force, player)
	...
	if (player) then
		local ret, msg = hook.Call(
			"PlayerUpdateContainerContents", GAMEMODE, player, entity, id, amount, force
		);
		if (ret ~= nil) then
			return ret, msg;
		end
	...
	if player then
		...
		hook.Call(
			"PlayerUpdatedContainerContents", GAMEMODE, player, entity, id, amount, force
		)
	end
	return true
end
...
concommand.Add(
	"MommyIAmDoneWithTheContainerNow", function(player)
		hook.Call("PlayerClosedContainerWindow", GAMEMODE, player)
	end
)
