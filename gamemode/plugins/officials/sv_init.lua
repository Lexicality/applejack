...
function PLUGIN:PlayerAdjustRadioRecipients(ply, text, recipients)
	...
	for _, teamdata in pairs(self.group.Teams) do
		table.Add(recipients, team.GetPlayers(teamdata.TeamID));
	end
end
...
function PLUGIN:PostPlayerSpawn(ply, light, teamchange)
	...
	timer.Create("Spawn Immunity: " .. ply:UniqueID(), duration, 1, tmr, ply); -- Make a timer to stop the immunity later
end
...
function PLUGIN:PlayerCanWarrant(ply, target, class)
	...
	if (team.NumPlayers(TEAM_MAYOR) > 0) then -- If there's a mayor and we're not him, we gotta beg.
		if (self:IsAuthorised(ply)) then
			ply:SayRadio(
				"Mayor, could you warrant " .. target:Name() .. " for " .. words ..
					" please?"
			);
		else
			self:SayRequest(
				ply,
				"Mayor, I suggest you warrant " .. target:Name() .. " for " .. words .. "."
			);
		end
	...
	if (team.NumPlayers(TEAM_POLICECOMMANDER) > 0) then -- If there's no mayor and we're not the commander, then we gotta pester the commander about it.
		if (self:IsAuthorised(ply)) then
			ply:SayRadio(
				"Commander, could you warrant " .. target:Name() .. " for " .. words ..
					" please?"
			);
		else
			self:SayRequest(
				ply, "Commander, I suggest you warrant " .. target:Name() .. " for " ..
					words .. "."
			);
		end
	...
	if (team.NumPlayers(TEAM_POLICEOFFICER) > 0) then -- I guess there might be police officers for us to pester?
		if (self:IsAuthorised(ply)) then
			ply:SayRadio(
				"Could an offier warrant " .. target:Name() .. " for " .. words ..
					" please?"
			);
		else
			self:SayRequest(
				ply,
				"I suggest that an officer warrants " .. target:Name() .. " for " .. words ..
					"."
			);
		end
...
function PLUGIN:PlayerWarrantExpired(ply, class)
	...
	if (team.NumPlayers(TEAM_MAYOR) > 0) then
		team.GetPlayers(TEAM_MAYOR)[1]:SayRadio(
			"The " .. class .. " warrant for " .. ply:Name() .. " has expired."
		);
	elseif (team.NumPlayers(TEAM_POLICECOMMANDER) > 0) then
		team.GetPlayers(TEAM_POLICECOMMANDER)[1]:SayRadio(
			"The " .. class .. " warrant for " .. ply:Name() .. " has expired."
		);
	elseif (team.NumPlayers(TEAM_POLICEOFFICER) > 0) then
		team.GetPlayers(TEAM_POLICEOFFICER)[1]:SayRadio(
			"The " .. class .. " warrant for " .. ply:Name() .. " has expired."
		);
	end
end
...
local doors = {
	{
		...
		Angle(000, 000, 000),
		Vector(-6866.8232421875, -9445.345703125, 73.03125),
		"Lift Access",
	},
	{
		"prop_dynamic",
		"models/props_lab/elevatordoor.mdl",
		Angle(000, 000, 000),
		Vector(-6768.6274414063, -9500.35546875, 841.03125),
		"Cells + Commander's Office",
	},
	{
		"prop_dynamic",
		"models/props_lab/elevatordoor.mdl",
		Angle(000, 000, 000),
		Vector(-7552.4541015625, -9159.3251953125, 841.03125),
		"Cells",
	},
	{
		"prop_dynamic",
		"models/props_lab/elevatordoor.mdl",
		Angle(000, 090, 000),
		Vector(-6919.013671875, -9440.412109375, 1481.03125),
	...
	},
	{
		...
		"models/props_lab/elevatordoor.mdl",
		Angle(000, 090, 000),
		Vector(-6829.3862304688, -9431.8671875, 2617.03125),
		"Mayor's Suite",
	},
	{
		"prop_dynamic",
		"models/props_lab/elevatordoor.mdl",
		Angle(000, 000, 000),
		Vector(-6965.9765625, -9148.5576171875, 2489.03125),
		"Mayor's Office",
	},
}
function PLUGIN:InitPostEntity()
	if (game.GetMap():lower() ~= "rp_evocity_v2d") then
		return
	end
...
function PLUGIN:PlayerCanDemote(ply, target)
	if (ply:Team() == TEAM_MAYOR and self:IsAuthorised(target)) then
		return true;
	elseif (not ply:IsModerator() and ply:Team() == TEAM_POLICECOMMANDER and
		target:Team() == TEAM_MAYOR) then
		return false, "You cannot demote the mayor!";
	end
end
...
GM:RegisterCommand{
	...
	function(ply)
		...
		elseif (ply:Team() == TEAM_MAYOR) then
			plugin:SayBroadcast(
				ply, "A lockdown is in progress. Please return to your homes."
			);
