...
local function UniqueIDToPhoneNumber(uniqueID)
	...
	return string.sub(uniqueID, 1, math.floor(length / 2)) .. "-" ..
	...
	if (target) then
		...
		if (target.phoneCalling) then
			...
			if (canAfford) then
				...
				ply:Notify(
					"(Phone) Stopped calling with " .. UniqueIDToPhoneNumber(target:UniqueID())
				);
				target:Notify(
					"(Phone) Stopped calling with " .. UniqueIDToPhoneNumber(ply:UniqueID())
				);
...
cider.command.add(
	"phone", "b", 1, function(ply, action, targetnumber)
		if (not ply._Phone) then
			ply:Notify("You don't have a phone!", 1);
			return;
		end
			...
			if (canAfford) then
				if (not ply.phoneCalling and not ply.phoneAnswer) then
					if (targetnumber and string.find(targetnumber, "-")) then
						ply:Notify(
							"(Phone) Now calling " .. targetnumber .. " (Wait for them to answer)"
						);
						...
						if (IsValid(target)) then
							if (not target.phoneCalling and not target.phoneAnswer and ply ~= target) then
								...
								target:Notify(
									"(Phone) Incomming call from " .. phoneNumber ..
										" ('/phone answer|deny' to answer or deny)"
								);
							...
							ply:Notify(
									...
									"(Phone) The person you are trying to call isn't available right now. Their phone might be turned off."
							);
						end
					else
		...
		elseif (action == "answer") then
			if (IsValid(ply.phoneAnswer)) then
				ply:Notify(
					"(Phone) Answered to phone call " ..
						UniqueIDToPhoneNumber(ply.phoneAnswer:UniqueID()) ..
						" (local cost is $2 per message sent)"
				);
				ply.phoneAnswer:Notify(
					"(Phone) Phone call was answered (local cost is $2 per message sent)"
				);
		...
		elseif (action == "deny") then
			if (IsValid(ply.phoneAnswer)) then
				ply:Notify(
					"(Phone) Denied phone call from " ..
						UniqueIDToPhoneNumber(ply.phoneAnswer:UniqueID())
				);
				ply.phoneAnswer:Notify(
					"(Phone) Phone call from " .. phoneNumber .. " denied.."
				);
		...
		elseif (action == "endcall") then
			if (IsValid(ply.phoneCalling)) then
				ply:Notify(
					"(Phone) Stopped calling with " ..
						UniqueIDToPhoneNumber(ply.phoneCalling:UniqueID())
				);
				ply.phoneCalling:Notify(
					"(Phone) Phone call with " .. phoneNumber .. " ended.."
				);
		...
		elseif (action == "getnumber") then
			...
			ply:Notify(
				"(Phone) Invalid action, you can only use call, endcall or getnumber!"
			);
		end
	end, "Commands", "<call|answer|endcall|getnumber> [phone number]",
	"Use call to start calling, anything you say will be heard through the phone.",
	true
);
