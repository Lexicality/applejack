...
GM:RegisterCommand{
	...
	function(ply, action, name)
		...
		if (action == "add") then
			...
			ply:Notify(
				"You have added a spawnpoint for " .. target.Name ..
			...
			if (count > 0) then
				ply:Notify(
					"You removed " .. count .. " " .. target.Name ..
						" spawnpoints from where you were looking, leaving " .. left .. " left."
				);
			else
				ply:Notify(
					"There are no " .. target.Name .. " spawnpoints where you are looking!"
				);
			end
			if (left == 0) then
				points[index] = nil;
