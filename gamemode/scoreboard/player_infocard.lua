...
function PANEL:UpdatePlayerData()
	...
	self:SetInfo(
		2, "Props:",
		self.Player:GetCount("props") + self.Player:GetCount("ragdolls") +
			self.Player:GetCount("effects")
	)
...
function PANEL:Think()
	...
	if (self.PlayerUpdate and self.PlayerUpdate > CurTime()) then
		return
	end
...
function PANEL:PerformLayout()
	...
	if (not self.Player or self.Player == LocalPlayer() or
	...
	if not IsValid(self.Player) or self.Player == LocalPlayer() or
		not hook.Call("PlayerCanDemote", GAMEMODE, LocalPlayer(), self.Player) then
