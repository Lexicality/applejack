...
GM:RegisterCommand{
	...
	function(ply, time)
		...
		timer.Create(
			ply:Name() .. " admin abuse knockout", 0.1, #tbl, knockoutfunct, tbl,
			time or 5
		);
	end,
};
...
GM:RegisterCommand{
	...
	function(ply)
		...
		timer.Create(
			ply:Name() .. " admin abuse unknockout", 0.1, #tbl, unknockoutfunct, tbl
		);
	end,
};
...
GM:RegisterCommand{
	...
	function(ply, victim, name, amount, force)
		...
		elseif (amount * item.Size > 50) then -- Something that not everyone bears in mind. (Including me occasionally.)
			ply:Notify(
				"Warning: You are giving " .. victim:Name() ..
					" more items than players can normally fit in their inventories. Experience has shown this if often a bad idea if done to anyone not part of the cabal. Remember you can remove items with negative numbers."
			);
		end
		...
		if (ply ~= victim) then
			person = victim:Name();
			victim:Notify(
				ply:Name() .. " has given you " .. amount .. " " .. name .. ".", 0
			);
		end
		player.NotifyByAccess(
...
GM:RegisterCommand{
	...
	function(ply, victim, flags)
		...
		player.NotifyAll(
			NOTIFY_CHAT,
			"%s gave %s access to the %q flag" .. (flags:len() > 1 and "s" or ""),
			ply:Name(), victim:Name(), flags
		);
	end,
};
...
GM:RegisterCommand{
	...
	function(ply, victim, flags)
		...
		player.NotifyAll(
			NOTIFY_CHAT,
			"%s removed %s's access to the %q flag" .. (flags:len() > 1 and "s" or ""),
			ply:Name(), victim:Name(), flags
		);
	end,
};
...
local function getnamething(kind, thing)
	...
	if kind == "team" then
		...
		if not team then
			return false, thing .. " is not a valid team!"
		elseif not team.Blacklist then
			return false, team.Name .. " isn't blacklistable!"
		end
	...
	elseif kind == "cmd" then
		...
		return false,
       		thing .. " is not a valid blacklist type! Valid: team/item/cat/cmd"
	end
end
...
GM:RegisterCommand{
	...
	function(ply, victim, kind, thing, time, reason)
		if (victim:IsModerator()) then
			...
			elseif (ply:IsModerator()) then
				if (victim:IsSuperAdmin()) then
					...
					return false,
       					"You cannot blacklist other members of the administration team!";
				end
			end
		end
		if (time < 1) then
			return false, "You cannot blacklist for less than a minute!";
		elseif ((time > 10080 and not ply:IsSuperAdmin()) or
			(time > 1440 and not ply:IsAdmin())) then
		...
		if (not gamemode.Call(
			"PlayerCanBlacklist", ply, victim, kind, thing, time, reason
		)) then
			return false;
		end
		...
		player.NotifyAll(
			NOTIFY_CHAT, "%s blacklisted %s from using %s for %s for %q.", ply:Name(),
			victim:Name(), name, time, reason
		);
	end,
};
...
GM:RegisterCommand{
	...
	function(ply, victim, kind, thing)
		...
		player.NotifyAll(
			NOTIFY_CHAT, "%s unblacklisted %s from using %s.", ply:Name(), victim:Name(),
			name
		);
	end,
};
			...
			if (table.Count(btab) == 0) then
				...
				for thing in pairs(btab) do
					...
					if (time ~= 0) then
						...
						time = getBlacklistTime(time);
						if (name:len() > namelen) then
							namelen = name:len();
						end
						if (admin:len() > adminlen) then
							adminlen = admin:len();
						end
						if (time:len() > timelen) then
							timelen = time:len();
						end
...
GM:RegisterCommand{
	...
	function(ply, victim, reason)
		...
		victim:Demote();
		player.NotifyAll(
			NOTIFY_CHAT, "%s demoted %s from %s for %q.", ply:Name(), victim:Name(),
			team.GetName(tid), reason
		);
	end,
};
...
GM:RegisterCommand{
	...
	function(ply, amt)
		...
		if (not (IsValid(victim) and victim:IsPlayer() and
			tr.StartPos:Distance(tr.HitPos) < 128)) then
...
GM:RegisterCommand{
	...
	function(ply, amt)
		...
		if ((ply._NextMoneyDrop or 0) > CurTime()) then
			return false,
       			"You need to wait another " .. (ply._NextMoneyDrop - CurTime()) ..
       				" seconds before dropping more money.";
		end
...
GM:RegisterCommand{
	...
	function(ply, identifier)
		...
		elseif (teamdata.SizeLimit ~= 0 and team.NumPlayers(teamid) >
			...
			if ((ply._NextUseItem or 0) > time) then
				return false,
       				"You cannot use another item for " ..
       					math.ceil(ply._NextUseItem - time) .. " more seconds!";
			elseif ((ply._NextUse[id] or 0) > time) then
				return false, "You cannot use another " .. item.Name .. " for " ..
       					math.ceil(ply._NextUse[id] - time) .. " more seconds!";
			end
...
local function containerHandler(ply, item, action, number)
	...
	if not (IsValid(container) and cider.container.isContainer(container) and
	...
	local cInventory, io, filter = cider.container
                               		.getContents(container, ply, true)
	...
	GM:RegisterCommand{
		...
		function(ply)
			...
			if (ply._NextHolsterWeapon and ply._NextHolsterWeapon > CurTime()) then
				return false, "You cannot holster this weapon for " ..
       					math.ceil(ply._NextHolsterWeapon - CurTime()) .. " second(s)!";
			else
...
GM:RegisterCommand{
	...
	function(ply, action, name)
		...
		if (not (IsValid(ent) and ent:IsOwnable() and tr.StartPos:Distance(tr.HitPos) <
			128)) then
		...
		if (name) then
			...
			if (lower:find("for sale") or lower:find("f2") or lower == "nobody") then
				name = ply:Name() .. "'s " ..
       					(ent._isDoor and "door" or ent:GetNWString("Name", "thingie"));
			end
		else
			name = ply:Name() .. "'s " ..
       				(ent._isDoor and "door" or ent:GetNWString("Name", "thingie"));
		...
		elseif (not (gamemode.Call("PlayerCanOwnDoor", ply, ent) and
			ply:CheckLimit("doors"))) then
			return false;
		end
		-- Capitalism, Ho!
...
GM:RegisterCommand{
	...
	function(ply, ...)
		local tr = ply:GetEyeTraceNoCursor();
		local ent = tr.Entity
		if (not (IsValid(ent) and ent:IsOwnable() and ply:GetPos():Distance(tr.HitPos) <
			128)) then
...
GM:RegisterCommand{
	...
	function(ply, itemid)
		...
		elseif (not ply:IsAdmin() and (ply.NextManufactureItem or 0) > CurTime()) then
			return false, "You cannot manufacture another item for " ..
       				math.ceil(ply.NextManufactureItem - CurTime()) .. " second(s)!";
...
local function canmutiny(ply, victim)
	...
	return
		pt and vt and pt.Group == vt.Group and pt.Gang == pt.Gang and pt.Gang and
			vt.GroupLevel == GROUP_GANGBOSS;
end
GM:RegisterCommand{
	Command = "mutiny",
	...
	function(ply, victim)
		...
		if (i < minimum) then
			ply:Notify(
				"Your vote has been counted, but you are not yet in the majority..."
			);
			return;
		end
		player.NotifyAll(
			NOTIFY_CHAT, "%s was overthrown as leader of the %s!", victim:Name(),
			gang.Name
		);
